...
{	
	...
	void FixedUpdate () 
	{
		...
		if(Input.GetAxis("Mouse Y") != 0)
		{
			...
			if(yRotation > 0 && (transform.localEulerAngles.x < 75 || transform.localEulerAngles.x > 315))
			{
				transform.Rotate(Vector3.right, yRotation*Time.deltaTime, Space.Self);
			}
			
			if(yRotation < 0 && (transform.localEulerAngles.x < 85 || transform.localEulerAngles.x > 325))
			{
				transform.Rotate(Vector3.right, yRotation*Time.deltaTime, Space.Self);
			}
